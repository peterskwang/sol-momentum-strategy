...
from config import ATR_PERIOD, MAX_PORTFOLIO_RISK_PCT, MAX_PORTFOLIO_NOTIONAL_X, STOP_ATR_MULT, SYMBOLS, TRAIL_ATR_MULT
from execution.order_manager import OrderManager
from strategy.exit_manager import compute_initial_exits, handle_tp1_fill, handle_stop_fill, TradeState, update_trailing_stop
...
class Portfolio:
    def __init__(
        ...
        # OrderManager is the single source of truth for live vs paper mode.
        # live_trading is resolved from the CLI --live flag at startup,
        # NOT from the LIVE_TRADING env var, preventing silent real-order placement.
    ...
    def handle_fill_event(
        ...
        """Process a FILLED ORDER_TRADE_UPDATE event from the user data stream.

        Matches ``order_id`` against tracked TP1 and stop order IDs in trade
        state.  Delegates to exit_manager helpers so tp1_filled / remaining_qty
        / closed flags are kept consistent.  This prevents the price-based
        stop-check from firing a redundant reduceOnly market sell after Binance's
        STOP_MARKET already executed.
        """
        ...
        if tp1_order_id is not None and order_id == tp1_order_id:
            # Reconstruct lightweight TradeState so we can call the shared helper
            trade = self._dict_to_trade_state(symbol, trade_dict)
            handle_tp1_fill(trade, fill_price)
            trade_dict.update(trade.__dict__)
            self.logger.info(
                "TP1 fill event: %s @ %.4f — tp1_filled=True, remaining_qty=%.4f",
                symbol,
                fill_price,
                trade_dict["remaining_qty"],
            )
            # Cancel stale stop-loss and replace with trailing stop on remaining qty
            ...
            trade = self._dict_to_trade_state(symbol, trade_dict)
            handle_stop_fill(trade, fill_price)
            trade_dict.update(trade.__dict__)
            self.logger.info(
                "Stop fill event: %s @ %.4f — trade closed cleanly (no redundant API call)",
                symbol,
                fill_price,
            )
    ...
    def _close_position(self, symbol: str, trade_dict: Dict[str, Any]) -> None:
        """Mark trade as closed and move it to closed_trades.

        P&L is computed using ``remaining_qty`` (post-TP1 it is 50% of the
        original size, not the full entry qty).
        """
    ...
    def _dict_to_trade_state(symbol: str, d: Dict[str, Any]) -> TradeState:
        return TradeState(
            ...
            tp1_filled=d.get("tp1_filled", False),
    ...
    def run_signal_cycle(self, btc_regime: str) -> None:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                # Place TP1 limit order and store its ID for fill-event matching
                tp1_qty = qty * 0.5  # TP1_CLOSE_PCT
                ...
                # Place stop-loss order and store its ID
